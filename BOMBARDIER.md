...
> ⚠️ **Doc parcialmente desatualizado (Page Builder).** Este documento descreve um produto
> "Page Builder" (rotas `/bombardier/{projects,page-builder,ux-flow}`, `AIChat.tsx`, etc.)
> que **NÃO existe neste repo**. As superfícies Bombardier reais aqui são:
> `/bombardier/styleguide` (DS + foundations + componentes + ux-flows na sidebar) e os
> bridges locais (`review-bridge/`, `flow-bridge/`). Use este doc só como referência
> conceitual do método; ignore as rotas/arquivos do builder.
...
- **Validação**: `zod`
- **Fontes**: Geist (voz tipográfica única), Geist Mono (código), Material Symbols Rounded (ícones)
