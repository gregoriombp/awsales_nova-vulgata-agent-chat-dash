...
function PrereqRow({
  ...
  return (
    <div className="flex items-start gap-3.5 rounded-md border border-(--border-subtle) bg-(--bg-canvas) px-4 py-3.5">
      <span
        aria-hidden="true"
        className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-(--bg-surface) text-(--fg-secondary)"
      >
      ...
      <a
        ...
        className="mt-0.5 inline-flex shrink-0 items-center gap-1 body-xs font-medium text-(--accent-brand) hover:underline"
      >
        {helpLabel}
...
function NumberedItem({
  ...
  return (
    <li className="flex items-start gap-3.5">
      <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-(--fg-primary) body-xs font-medium text-white">
        {num}
      </span>
      <p className="m-0 body-sm text-(--fg-primary)">
        <strong className="font-medium">{head}</strong>
        <span className="font-normal text-(--fg-secondary)">{tail}</span>
      </p>
    </li>
  );
}
...
export default function WhatsAppSetupPage() {
  ...
  return (
    <AwDashboardLayout breadcrumbs={breadcrumbs}>
      <div className="-m-8 flex min-h-full flex-col bg-(--bg-canvas)">
        <header className="flex items-start justify-between gap-4 border-b border-(--border-subtle) bg-(--bg-canvas) px-10 py-6">
          <div className="flex items-center gap-3.5">
            <AwBrandLogo brand="whatsapp" size="lg" />
            <div>
              <h5 className="m-0 text-(--fg-primary)">
                Conectar WhatsApp
              </h5>
          ...
          <button
            type="button"
            onClick={exitSetup}
            className="inline-flex shrink-0 items-center gap-1.5 rounded-sm px-2 py-1.5 body-xs text-(--fg-secondary) transition-colors hover:bg-(--bg-surface) hover:text-(--fg-primary)"
          >
        ...
        <main className="flex-1 px-10 py-10 pb-32">
          <section className="mx-auto w-full max-w-[640px] rounded-xl border border-(--border-subtle) bg-(--bg-surface) px-10 py-12">
            ...
            <h4 className="m-0 mt-6 text-center text-(--fg-primary)">
              Conectar com a Meta Business
            </h4>
            <p className="m-0 mx-auto mt-2 max-w-[460px] text-center body-sm text-(--fg-secondary)">
            ...
            <Collapsible className="group mt-7 rounded-md border border-(--border-subtle) bg-(--bg-canvas)">
              <CollapsibleTrigger className="flex w-full items-center justify-between gap-3 rounded-md px-4 py-3 text-left body-xs font-medium text-(--fg-primary) transition-colors hover:bg-(--bg-surface)">
                ...
                <Icon
                  name="expand_more"
                  size={18}
                  className="text-(--fg-tertiary) transition-transform group-data-[state=open]:rotate-180"
                />
              </CollapsibleTrigger>
              <CollapsibleContent className="border-t border-(--border-subtle) px-4 py-4">
            ...
            <div className="mt-7 flex flex-wrap items-center justify-center gap-2 border-t border-(--border-subtle) pt-6">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-(--aw-emerald-50,#E6F9EE) px-2.5 py-1 body-xs font-medium text-(--aw-emerald-700)">
                <span className="h-1.5 w-1.5 rounded-full bg-(--aw-emerald-700)" />
                Conexão criptografada · OAuth 2.0
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-full border border-(--border-subtle) bg-(--bg-canvas) px-2.5 py-1 body-xs font-medium text-(--fg-secondary)">
        ...
        <footer className="sticky bottom-0 flex items-center justify-between gap-4 border-t border-(--border-subtle) bg-(--bg-canvas) px-10 py-4">
