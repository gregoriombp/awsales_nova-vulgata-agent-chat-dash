...
- MCPs imported from Cursor: `Figma` remote MCP (`https://mcp.figma.com/mcp`) and `shadcn` (`npx shadcn@latest mcp`). The project-level config lives in `.mcp.json`; OAuth tokens stay in the user's local app storage and must never be copied into this repo.
