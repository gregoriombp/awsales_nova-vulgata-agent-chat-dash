...
const PORT = Number(process.env.CLAUDE_EDIT_BRIDGE_PORT ?? 9877)
const VERSION = "0.1.0"

// cwd that the agent operates on. Defaults to the parent of bridge-edit/ (the repo root).
const REPO_CWD =
  process.env.CLAUDE_EDIT_CWD ?? path.resolve(process.cwd(), "..")
...
app.get("/health", async (_req, res) => {
  ...
  res.json({
    ...
    claude: {
      ...
      reason: claude.ready ? undefined : claude.reason,
    },
    cwd: REPO_CWD,
...
app.post("/chat", async (req, res) => {
  ...
  try {
    for await (const ev of runChat(fullReq) as AsyncIterable<StreamEvent>) {
      if (aborted) break
      switch (ev.kind) {
        case "status":
        ...
        case "tool_use":
          ...
          console.log(`[tool] ${ev.name}`, JSON.stringify(ev.input).slice(0, 200))
          break
        case "tool_result":
        ...
        case "result":
          writeSse(res, "result", {
            text: ev.text || lastAssistant || fullText,
...
app.listen(PORT, "127.0.0.1", async () => {
  const status = await getClaudeStatus(true)
  console.log("")
  console.log(`  ✏️   Claude edit bridge v${VERSION}`)
  console.log(`      http://localhost:${PORT}  (health + chat)`)
  console.log(`      cwd:          ${REPO_CWD}`)
  if (status.ready) {
