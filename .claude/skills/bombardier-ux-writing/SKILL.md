...
description: Passa um pente-fino de UX writing IN-PRODUCT numa rota, em várias, ou em links colados — lê as strings reais dos arquivos da página, audita cada uma contra a voz do PRODUTO AwSales (resolve, não vende), propõe reescritas com justificativa, espera aprovação e aplica de forma cirúrgica (só texto, nunca layout). A voz é inspirada em ElevenLabs (eficiência, simplicidade, clareza) e OpenAI (caloroso, conciso, confiante, nunca bajulador), adaptada ao PT-BR e ancorada no AWSALES_CONTEXT.md. Use quando o usuário disser "/bombardier-ux-writing", "aplica o ux writing", "revisa a copy dessa página", "olha o texto/microcopy de /rota", "melhora o ux writing", "revisa os labels/erros/empty states", "deixa essa tela no tom do produto", ou colar uma rota/link/texto pedindo revisão de escrita de interface. NÃO é voz de marketing/site (isso é `awsales-brand-voice`) e NÃO mexe em layout/estrutura (isso é `ux-page-rework`).
