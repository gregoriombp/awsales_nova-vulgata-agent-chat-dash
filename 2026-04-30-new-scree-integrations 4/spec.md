...
**Providers da mesma marca (D-CARD5):** alguns providers do catálogo são entidades separadas no backend mas pertencem à mesma marca — ex: `hotmart` (categoria `checkout`) e `hotmart-club` (categoria `members_area`) são 2 `IntegrationProvider` distintos com mesma identidade visual. **Cliente vê 2 cards separados no grid** com mesmo logo, indicando claramente que são 2 conexões independentes (estados de saúde podem divergir — Hotmart checkout pode estar OK enquanto Hotmart Club expirou). No catálogo W3, aparecem em filtros de category diferentes (filtro `checkout` mostra Hotmart; filtro `members_area` mostra Hotmart Club).
...
└── Integrações                              ← rota principal
    ...
    │
Studio sidebar (irmãos):
├── Integrações (rota acima — Integration Hub: Hotmart, HubSpot, Hubla, etc.)
├── Canais (rota nova — Messaging domain: WhatsApp, Instagram, Messenger)
...
**Decisões de IA:**
- **Tools tem rota dedicada** (D-IA3) — não só dentro de conexão, porque é vetor de execução da IA com governança própria.
- **Canais é rota irmã** (D-IA9) — não filha de Integrações, porque WhatsApp/IG/Messenger pertencem ao **domínio Messaging** (não Integration Hub). Backend já separa; frontend agora reflete a separação real. WhatsApp não cabe em "+ Adicionar integração" porque o fluxo de criação é Meta Embedded Signup, não OAuth genérico.
...
- **Header da conexão**: logo do provider · nome dado pelo usuário · **3 chips separados** (D-DET3 — não 1 chip combinado):
  - Chip 1 — **Status interno** (`active`/`pending_auth`/`expired`/`revoked`/`error`)
  - Chip 2 — **Saúde** (`healthy`/`degraded`/`unreachable`) — diferente de status
  - Chip 3 (só pra WABA / providers Meta) — **Status na Meta** (`approved`/`pending_review`/`rejected`/`disabled`) + Quality rating (`high`/`medium`/`low`/`flagged`)
- timestamp último evento · ações (`Reconectar` / `Testar` / `Desassociar`)
...
**Por que 3 chips separados (D-DET3):** Paulo na call reportou que hoje o WhatsApp fica "Ativo" no header **mesmo quando a BM caiu** — porque chip combinado mente. Cliente não diferencia "AWsales perdeu acesso" de "Meta suspendeu o número" de "API do provider lenta". Cada um exige ação diferente — confundir = ação errada.
...
### 6.1.1 Banner de onboarding pós-criação (D-EST4 — ajustado pós-feedback PG)
...
- Checklist específico do provider (texto vem do seed-data `provider.onboardingChecklist`):
  - Ex Hotmart: "1. Cole essa URL em **Hotmart > Ferramentas > API > Webhook**" + "2. Marque eventos: `purchase_completed`, `subscription_*`" + "3. Salve e **faça uma compra teste de R$ 1 no Hotmart** pra disparar o primeiro webhook"
  - Ex HubSpot: "1. Configure webhook em **Settings > Integrations > Webhooks**" + ...
- Status ao vivo: "Aguardando primeiro evento real do provider..." → "Recebido em X" quando chega
- Botão `[Verificar agora]` (refresh) — força refetch sem reload da página
- Banner some quando primeiro webhook real chega

**Importante (D-EST4 ajustada pós-feedback PG):** ❌ **NÃO temos botão "Enviar evento de teste"**. Webhooks vêm sempre do provider — não simulamos. Pra Hotmart, cliente faz compra real (R$ 1) na Hotmart pra disparar. Pra HubSpot, cria/edita contato real. Pra checkouts com sandbox próprio (Stripe Test Mode), cliente usa lá e nós recebemos normal. Versão anterior do banner tinha "Enviar evento de teste" que prometia simulação inexistente — corrigido.

**Pra evento personalizado (W3a Webhook customizado):** mesma regra — quem dispara é o sistema do cliente (checkout próprio, n8n, make...). Modal pós-criação deixa explícito: "Não enviamos eventos de teste — o gatilho vem sempre do seu sistema".

**Pra `oauth2_*` providers que recebem webhook via OAuth scope automático** (HubSpot, etc.): banner skip a parte "cole URL" (não precisa) e mostra só checklist com instrução de ação real.
...
## 8. Canais (rota irmã de Integrações) — Messaging domain

> **D-IA9 (pós-feedback PG):** Canais é rota **irmã** de Integrações na sidebar do Studio, não filha. Backend separa em domínios diferentes (Messaging vs Integration Hub) — frontend agora reflete a separação. WhatsApp NÃO aparece mais no grid de "Minhas conexões" (W1).

**Sidebar do Studio:**
```
Integrações  ← Hotmart, HubSpot, Hubla, etc. (Integration Hub)
Canais       ← WhatsApp, Instagram (em breve), Messenger (em breve)  (Messaging)
Tools        ← cross-conexão IA actions
```

**Tela `Canais` (rota `/canais`):**
- Grid simples com canais habilitados na org (1 card por tipo de canal: WhatsApp, IG, Messenger)
- Cada card → leva pra detalhe do canal (W11 pra WhatsApp = lista de WABAs)
- Botão `+ Adicionar canal` no header (fluxos diferentes por tipo: Meta Embedded Signup pra WABA, etc.)
...
3. Cliente cria → backend gera `webhookUrl` + `webhookSecret`. Modal final mostra ambos com copy-button + aviso "secret não-recuperável" + **aviso explícito: "Não enviamos eventos de teste — o gatilho vem sempre do seu sistema"**.
4. Cliente cola URL no sistema dele (middleware Shopify, código próprio, n8n, make...) → faz a primeira ação real (compra teste no checkout dele) → primeiro webhook chega na AwSales.
5. Banner de onboarding (D-EST4) cobre o "como saber que chegou" (ver §6.1.1).

**Importante:** AwSales **nunca** envia eventos de teste sintéticos pra esse webhook. Se cliente quer testar, ele faz POST manual via curl/Postman/seu sistema. Modal pós-criação inclui link "Ver schema esperado" com docs de payload.
