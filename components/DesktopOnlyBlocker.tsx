...
export function DesktopOnlyBlocker({ children }: { children: React.ReactNode }) {
  ...
  if (mounted && tooSmall) {
    return (
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-[var(--bg-canvas)] px-8 text-center">
        <div className="flex max-w-[420px] flex-col items-center gap-5">
          <span className="flex h-14 w-14 items-center justify-center rounded-[var(--radius-lg)] bg-[var(--bg-inverse)] text-[var(--fg-on-inverse)]">
            <Icon name="desktop_windows" size={28} />
          </span>
          <div className="flex flex-col gap-2">
            <h2 className="m-0 text-[var(--fg-primary)]">
