...
import { useMergeSplitBlocks, SelectionBackgrounds } from "@/components/ui/fluid/lib/use-merge-split";
import { Button } from "@/components/ui/fluid/button";

// Material Symbols wrappers in the shape Button's icon slots expect
// ({ size, className }) — strokeWidth is accepted for contract parity but
// font glyphs ignore it.
function ArrowLeftGlyph({
  ...
  return <Icon name="arrow_back" size={size} className={className} />;
}
...
function ArrowRightGlyph({
  ...
  return <Icon name="arrow_forward" size={size} className={className} />;
}
...
export interface AskUserQuestionsProps
  ...
  skipLabel?: string;
  /** Label for the Back footer button. */
  backLabel?: string;
  /** Label for the multi-select Continue button (non-final questions). */
  continueLabel?: string;
  /** Label for the multi-select Continue button on the last question. */
  finishLabel?: string;
  /** Header counter, e.g. (1, 3) => "Question 1 of 3". */
  progressLabel?: (current: number, total: number) => ReactNode;
  /** Text shown when `questions` is empty. */
  emptyLabel?: string;
  /** Fallback placeholder for the "Other" free-text row (a question's own
   *  `otherPlaceholder` wins). */
  otherPlaceholder?: string;
}
  ...
  function AskUserQuestions(
    {
      ...
      otherPlaceholder: otherPlaceholderProp = "Describe in your own words…",
      className,
      ...rest
    },
    ref
    ...
    const otherPlaceholder = question?.otherPlaceholder ?? otherPlaceholderProp;
    ...
    // mouse hover (activeIndex → bg-bg-hover), so keyboard and pointer focus
    // look identical. Left = Back, Right = Skip. We stopPropagation on the
    // arrows we handle so the doc page's ←/→ page-change nav (a window
    // listener) doesn't also fire — important for multi-select, whose
    // container is role="group" (not "radiogroup") and so isn't auto-skipped
    // by that handler.
    ...
    if (!question) {
      return (
        <div
          ref={ref}
          className={cn(
            "w-full max-w-130 p-5 bg-bg-raised border border-border",
            shape.container,
            className
          )}
          {...rest}
    ...
    // The morphing focus ring is intentionally suppressed for the Other
    ...
    // selected block with adjacent selected options.
    ...
    return (
      <div
        ...
        className={cn(
          ...
          "relative w-full max-w-130 overflow-hidden bg-bg-raised border border-border",
          shape.container,
          className
        )}
        {...rest}
      ...
      >
        ...
        <div className="flex items-center px-5 pt-5 pb-2 text-xs text-fg-tertiary">
          <span>
            {progressLabel
        ...
        >
          <div
            ...
            className={cn("px-5", showFooter ? "pb-1" : "pb-3")}
          >
            <div key={qId} className="flex flex-col gap-2">
            {/* Question title */}
            <h3
              id={`${reactId}-${qId}-title`}
              className="text-base text-fg-primary leading-snug"
            ...
            >
                ...
                {(() => {
                  ...
                  return (
                    <motion.div
                      ...
                      aria-hidden
                      className={cn(
                        "absolute pointer-events-none bg-bg-raised ring-1 ring-inset ring-border",
                        shape.bg
                      )}
                      initial={{
                        opacity: 0,
              ...
              <AnimatePresence>
                {activeRect && (
                  <motion.div
                    ...
                    aria-hidden
                    className={cn(
                      "absolute pointer-events-none bg-bg-hover",
                      shape.bg
                    )}
                    initial={{
                      opacity: 0,
              ...
              {/* Selected-row backgrounds (merged for contiguous selections).
                  ...
                  abutting halves — see useMergeSplitBlocks. Renders ABOVE the
                  hover indicator so the selected state stays readable when
                  mousing over a row. Corners are driven numerically (around
                  shape.bg's radius) so a single selected row matches its hover. */}
              ...
              <AnimatePresence>
                {focusRect && (
                  <motion.div
                    aria-hidden
                    className={cn(
                      "absolute pointer-events-none border border-(--ring-focus) z-20",
                      shape.focusRing
                    )}
                    initial={{
                      opacity: 0,
              ...
              {options.map((opt, i) => {
                ...
                return (
                  <Row
                    ...
                    arrowIcon={<Icon name="arrow_forward" size={14} />}
                  >
                    {question.layout === "stacked" ? (
                      <>
                        <span className="inline-grid">
                          ...
                          <span
                            className="col-start-1 row-start-1 text-fg-primary transition-[color,font-variation-settings] duration-80"
                            style={{
                  ...
                  <span className="inline-grid w-full">
                    <textarea
                      ...
                      onKeyDown={(e) => {
                        ...
                        // Shift+Enter inserts a newline. In multi-select we
                        // leave plain Enter to the textarea (newline) and let
                        // the root handler catch ⌘/⌃+Enter for Continue —
                      ...
                      className={cn(
                        ...
                        "col-start-1 row-start-1 block w-full bg-transparent border-0 p-0 m-0 outline-none resize-none overflow-hidden text-sm leading-snug text-fg-primary placeholder:text-fg-muted"
                      )}
                  ...
                  {showBack && (
                    ...
                    >
                      {/* Bare ← icon via the Button's icon slot, so it gets the
                          proper tighter icon-side padding. The arrow hints at
                          the ArrowLeft shortcut. */}
                      <Button
                        variant="ghost"
                        size="sm"
              ...
              <div className="relative flex items-center gap-2">
                <AnimatePresence mode="popLayout" initial={false}>
                  {showSkip && (
                    ...
                    >
                      {/* Bare → icon via the Button's icon slot (mirror of
                          Back). Hints at the ArrowRight shortcut. */}
                      <Button
                        variant="ghost"
                        size="sm"
...
function ShortcutChip({
  ...
  return (
    <kbd
      aria-hidden
      className={cn(
        "inline-flex items-center justify-center gap-0.5 px-1 min-w-4.5 h-4.5 text-xs leading-none font-sans tracking-wide",
        tone === "inverted"
          ? "bg-fg-on-inverse/15 text-fg-on-inverse"
          : "bg-fg-primary/10 text-fg-tertiary",
        shape.bg
      )}
    >
      {children}
...
function Row({
    ...
    <AnimatePresence>
      {showArrow && (
        <motion.span
          ...
          className={cn(
            "absolute inset-0 inline-flex items-center justify-center bg-bg-inverse text-fg-on-inverse",
            shape.bg,
  ...
  // lines up with the centre of a `text-sm leading-snug` first line
  ...
  const chipSlot = (
    ...
    >
      <span
        aria-hidden
        className={cn(
          "absolute inline-flex items-center justify-center w-5 h-5 text-xs transition-[opacity,font-variation-settings] duration-80",
          isMulti && shape.bg,
          isMulti
            ? chipFilled
