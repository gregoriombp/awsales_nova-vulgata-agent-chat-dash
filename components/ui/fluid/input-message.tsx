...
import { SurfaceProvider } from "@/components/ui/fluid/lib/surface-context";
import { FileThumbnail } from "@/components/ui/fluid/file-thumbnail";
...
interface InputMessageProps
  ...
  placeholder?: string;
  /** Placeholder swap shown while dragging files over the composer. */
  dropPlaceholder?: string;
  /** Label for the remove (×) affordances on preview tiles and queued rows. */
  removeLabel?: string;
  /** Fallback label for a queued row with attachments and no text,
   *  e.g. (2) => "2 attachments". */
    ...
    >
      ...
      <Tooltip content={removeLabel} side="top">
        <button
          ...
          // Force a fixed dark circle + light × regardless of theme — the
          // close badge needs to read as a "delete affordance" over arbitrary
          // image/PDF content, so it sits at a fixed contrast instead of
          // flipping with the surrounding surface.
          className="absolute top-1 right-1 w-5 h-5 rounded-full bg-aw-gray-1100 text-aw-gray-25 opacity-0 group-hover/tile:opacity-100 transition-opacity duration-80 flex items-center justify-center cursor-pointer outline-none focus-visible:opacity-100 focus-visible:ring-1 focus-visible:ring-(--ring-focus)"
        >
...
function QueuedRow({
  ...
  const label = item.text || attachmentsLabel(fileCount);
  ...
  return (
    <Reorder.Item
      ...
      className={cn(
        "group/qrow flex items-center gap-2 rounded-lg bg-bg-muted px-2.5 py-1.5",
        "text-sm text-fg-secondary select-none outline-none",
    ...
    >
      {fileCount > 0 && (
        <span className="flex shrink-0 items-center gap-0.5 text-fg-tertiary">
  ...
  (
    {
      ...
      dropPlaceholder = "Drop files here to add to chat",
      removeLabel = "Remove",
      attachmentsLabel = (n) => `${n} attachment${n === 1 ? "" : "s"}`,
      leftSlot,
      rightSlot,
      disabled,
      minRows = 1,
    ...
    // border band layered beside it). The drop (--shadow-sm) is kept so the
    // composer holds its lift across states. Applied inline (not via a
    // Tailwind `shadow-*` utility, which mangles multi-layer arbitrary
    // values) with the precedence drag > focus > hover; when none are active,
    // the className's surface shadow supplies the resting edge.
    const EDGE_DROP = "var(--shadow-sm)";
    const edgeShadow = dragOver
      ? `0 0 0 1px var(--ring-focus), ${EDGE_DROP}`
      : focusVisible
        ? `0 0 0 1px color-mix(in srgb, var(--fg-primary) 20%, transparent), ${EDGE_DROP}`
        : hovered && clickToFocus && !disabled
          ? `0 0 0 1px var(--border-default), ${EDGE_DROP}`
          : undefined;
    ...
    return (
      <div
        ...
        onDrop={handleDrop}
        className={cn(
          // The edge is the box-shadow's hairline ring (from the surface), not
          // a border. State changes recolor that same 1px ring in place rather
      ...
      >
        <SurfaceProvider value={2}>
          ...
          <textarea
            ...
            placeholder={
              dragOver && supportsFiles ? dropPlaceholder : placeholder
            }
            disabled={disabled}
            rows={minRows}
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              >
                <AnimatePresence mode="wait" initial={false}>
                  ...
                  >
                    {buttonMode === "stop" ? (
                      <Icon name="stop" fill={1} size={18} />
                    ) : (
                      // Slightly larger than the icon-sm default — the send
                      // glyph reads better with a touch more presence.
