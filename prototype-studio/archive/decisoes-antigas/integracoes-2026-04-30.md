...
### D-EST4 — Banner de onboarding pós-criação ✅ (ajustado pós-feedback PG 2026-04-30)
...
**Alternativas consideradas.**
- **A. Sem onboarding** — Contras: tickets explodem.
- **B. Sandbox / test mode separado** — Prós: pattern Stripe. Contras: caro (ambiente staging duplo).
- **C. Banner contextual + send test event sintético** — versão original
- **D. Banner contextual + checklist + "Verificar agora" (sem test event sintético)** ← versão FINAL

**Por que D é superior.** Mecânica:
- Banner aparece quando `lastWebhookReceivedAt = null` E conexão tem `<24h`.
- Mostra: URL + secret + checklist específico do provider + status ao vivo.
- **SEM botão "Enviar evento de teste"** — feedback PG: "não enviamos evento de teste, é o webhook do Hotmart enviado pra nós". Backend não simula webhook do provider sem ser pelo provider. UX prometia feature inexistente — corrigido.
- **Pra evento personalizado** (W3a webhook customizado): mesma regra — quem dispara é o sistema do cliente. Modal pós-criação deixa explícito.
- **Botão "Verificar agora"** (refresh) substitui — força refetch dos últimos webhooks recebidos sem reload da página.
- Instrução acionável no checklist: "Faça uma compra teste de R$ 1 no provider".

**Trade-off.** Cliente sem ação no provider fica esperando. Mitigado por: instrução clara no checklist + provedores como Stripe têm sandbox próprio que dispara webhooks reais (cliente usa lá).

**Quando reconsiderar.** Se backend implementar replay de evento histórico, adicionar `[Replay último evento]` no banner — diferente de "enviar de teste" porque o evento original veio do provider real.
...
## Decisões adicionadas pós-feedback PG (protótipo · 2026-04-30)

### D-IA9 — Canais é rota irmã de Integrações (não filha) ✅

**Problema.** Versão anterior listava WhatsApp dentro do grid de "Minhas conexões" (W1) junto com Hotmart/HubSpot/etc. Feedback PG: "canais faz sentido listar junto com integrações? contextos bem diferentes, não tem personalizado etc..."

**Alternativas consideradas.**
- **A. Canais dentro de Integrações (versão anterior)** — Contras: WABA não tem objetos catalogáveis no sentido de Hotmart, não tem auth OAuth bidirecional, não cabe em "Tools custom". Fluxo de criação é Meta Embedded Signup, completamente diferente. Categoria `marketplace` no card não bate com a realidade do canal. Mistura 2 domínios (Integration Hub + Messaging) em 1 lista.
- **B. Canais como rota irmã (sidebar separada)** ← escolhida

**Por que B é superior.** Mecânica:
- Backend já separa: WABA é `ChannelAccount` em **Messaging domain**; Hotmart/HubSpot são `IntegrationConnection` em **Integration Hub**. Frontend agora reflete a separação real.
- Sidebar do Studio: `Integrações` · `Canais` · `Tools` (3 itens irmãos), em vez de WABA escondido como sub-item de Integrações.
- W1 (Minhas conexões) só lista Integration Hub (Hotmart, HubSpot, DMG, Hubla, etc.). WhatsApp some do grid.
- Nova rota `/canais` lista os canais (WhatsApp v1; Instagram + Messenger v1.5 marcados como "em breve").
- Catálogo W3 não tem mais a "ginástica" de incluir WABA como categoria estranha.
- Operador (Card, Daniel, Paulo) pensa "canal" e "integração" como conceitos diferentes — UX casa com modelo mental.

**Trade-off.** 1 item a mais na sidebar do Studio. Mitigado: separação clara reduz confusão futura quando entrar Email/SMS (também canais Messaging).

**Quando reconsiderar.** Sem motivo. Decisão arquitetural alinhada com domain boundaries do dev.

---

### D-CARD5 — Providers da mesma marca aparecem como conexões separadas (com logo compartilhado) ✅

**Problema.** Backend tem `Hotmart` (checkout) e `Hotmart Club` (members_area) como **2 providers seed-data separados** — confirmado em [providers.mdx](../../apps/awsales-domains-contracts/domain/integration-hub/providers.mdx). Cliente que usa as 2 vê 2 cards no grid. Pergunta PG: "no caso da Hotmart que também é área de membro, aparecer nas duas categorias, correto? adicionar exemplo na tela inicial."

**Alternativas consideradas.**
- **A. 1 card "Hotmart" agrupado com 2 capabilities** — Prós: 1 marca = 1 card. Contras: backend são entidades separadas (UNIQUE por providerId). Estados de saúde podem divergir (checkout funciona, members_area não). Esconde verdade.
- **B. 2 cards separados (Hotmart checkout + Hotmart Club members_area)** ← escolhida — com identidade visual de marca compartilhada (mesmo logo "H")
- **C. 2 cards + linha visual ligando-os** — Prós: explicita o vínculo. Contras: complexidade visual extra.

**Por que B é superior.** Mecânica:
- Cada provider seed-data tem `category` (`checkout` para Hotmart, `members_area` para Hotmart Club) — aparecem em filtros diferentes do catálogo.
- No grid de Minhas conexões: 2 cards distintos. Cliente vê estados independentes (Hotmart pode estar OK enquanto Hotmart Club expirou).
- **Compartilham logo** (`provider.logo='H'` nos dois) e indicação de marca (`provider.brand='hotmart'`). Cliente reconhece visualmente que são da mesma família.
- No catálogo W3: ao clicar Hotmart (checkout), drawer pode sugerir "Você também pode conectar Hotmart Club (área de membros)" como cross-sell — v1.5.
- Pareto: maioria dos clientes só tem 1 dos 2 (não ambos). Quando tem ambos, cards separados mostram independência saudável.

**Trade-off.** Cliente Power que tem várias integrações da mesma marca pode sentir pequena duplicação. Mitigado: filtro de busca acha "hotmart" e mostra os 2 juntos (substring match).

**Quando reconsiderar.** Se aparecerem providers que **realmente** são "1 só" mas o backend modela errado (ex: HubSpot CRM + HubSpot Marketing como providers separados quando deveria ser 1), promover pra "agrupado". Por enquanto, todos os pares conhecidos justificam separação.

---

## Decisões revisitadas após Stress Test 2

⚠️ **4 decisões ajustadas/revertidas:**
...
- **D-EST4** AJUSTADA pós-feedback PG (2026-04-30) → removido botão "Enviar evento de teste" (não simulamos webhook do provider) — substituído por "Verificar agora" + checklist com instrução de ação real no provider

---

## Resumo executivo

**34 decisões totais** após feedback PG do protótipo (era 32, era 25 antes do Stress Test 2). Todas com defesa concreta (mecânica, não slogan).
...
**Mudanças pós-feedback PG no protótipo (2026-04-30):**
- 2 decisões novas:
  - **D-IA9** — Canais é rota irmã de Integrações (não filha)
  - **D-CARD5** — Providers da mesma marca (Hotmart + Hotmart Club) aparecem como conexões separadas com logo compartilhado
- 1 decisão ajustada:
  - **D-EST4** — removido botão "Enviar evento de teste" (UX prometia feature inexistente — webhooks vêm sempre do provider, não de simulação nossa)
- 1 decisão melhor aplicada visualmente:
  - **D-DET3** — agora 3 chips separados (status × saúde × Meta) no header da conexão, em vez de chip único combinado (era a decisão certa, faltava aplicação no protótipo)
